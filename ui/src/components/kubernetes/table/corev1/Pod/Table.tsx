      ...
      withNamespacedResourceColumns(
        [
          ...
          },
          {
            ...
            accessorFn: (row) => `${row.metadata?.namespace}/${row.metadata?.name}`,
            cell: ({ getValue }) => (
              <MetricUsageCell podKey={getValue() as string} format="memory" />
            ),
            size: 80,
          },
          {
          ...
          },
          {
            ...
            header: 'Status',
            accessorFn: (row) => {
              return row.metadata?.deletionTimestamp ? 'Terminated' : row.status?.phase;
            },
      ...
      actions: [
        {
          ...
          action: (ctx) => {
            void show({
              ...
              body: (
                <span>
                  Are you sure you want to delete the Pod <code>{ctx.data?.metadata?.name}</code>{' '}
                  from <strong>{ctx.data?.metadata?.namespace}</strong>?
                </span>
              ),
              confirmLabel: 'Delete',
              cancelLabel: 'Cancel',
